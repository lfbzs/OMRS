...
def make_track(_score,meta_time,program=0):
    ...
    for i in range(0,len(_score)):
        ...
        try:
            if int(_score[i+1][0]):
                track.append(Message('note_off', note=int(ch[0]), velocity=64, time=round(0.5*_score[i][1]*meta_time),channel=0))
        except Exception:
            pass
    # track.append(Message('note_off', note=int(_score[-1][0]), velocity=64, time=round(0.5*_score[-1][1]*meta_time),channel=0))
