...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    if save_txt or save_img:
        ...
        imgs_information = creat_txt(label_path, mung_path,list_img)

        xml_dir = os.path.join(save_dir, "xml/")
        if not os.path.isdir(xml_dir):
            os.mkdir(xml_dir)
        save_xml_file(label_path, list_img, xml_dir)
    if update:
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/UI/weights/last.pt', help='model path(s)')
